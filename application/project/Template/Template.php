...
class Template {
    ...
    /**
     * Obtiene el template, no lo imprime
     *
     * @param string $handle
     */
    function fetch($handle)
    {
        return $this->pparse($handle,true);        
    }
     ...
     */
    function pparse($handle,$do_not_echo = false)
    {
        ...
        if (!isset($this->compiled_code[$handle]) || empty($this->compiled_code[$handle]))
        {
            ...
            $this->compiled_code[$handle] = $this->compile($this->uncompiled_code[$handle],$do_not_echo);
        }
    ...
    {
        $this->assignBlock($blockname,array());
    }
     ...
     * iteration.
     */
    function assignBlock($blockname, $vararray)
    {
        if (strstr($blockname, '.'))
        {
            // Nested block.
     ...
     */
    function compile($code, $do_not_echo = false, $retvar = 'customTemplate')
    {
        ...
        for ($i = 0; $i < $line_count; $i++)
        {
            ...
            if (preg_match('#<block:(.*?)>#', $code_lines[$i], $m))
            {
                $n[0] = $m[0];
                $n[1] = $m[1];

                // Added: dougk_ff7-Keeps templates from bombing if begin is on the same line as end.. I think. :)
                if ( preg_match('#</block:(.*?)>#', $code_lines[$i], $n) )
                {
                    $block_nesting_level++;
            ...
            else if (preg_match('#</block:(.*?)>#', $code_lines[$i], $m))
            {
                // We have the end of a block.
                unset($block_names[$block_nesting_level]);
                $block_nesting_level--;
