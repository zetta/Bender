...
class CatalogGenerator extends ModelGenerator
{
  ...
  /**
   * Método que itera sobre Todos los items dentro del FieldCollection
   * @param FieldCollection $fields
   */
  public function loopAllFields()
  {
    $collections = array($this->table->getFields());
    if ($this->extends)
      array_push($collections, $this->table->getExtendedTable()->getFields());
    $assigned = array();
    foreach ( $collections as $fields )
    {
      ...
      {
        $field = $fields->current();
        if(isset($assigned[$field->getSimpleName()]))
        {
          $fields->next();
          continue;
        }
  ...
  private function loopOverForeignKeys()
  {
    ...
    while ( $fields->valid() )
    {
      ...
      $this->template->assignBlock('foreignKeys', array('return' => $field->isUnique() ? $this->object : $this->object . 'Collection', 'getOne' => $field->isUnique() ? '->getOne()' : '', 
          'fkConstant' => $field->getCatalogAccesor(), 'fkName' => $field->getPhpName(), 'fkType' => $field->getDataType(), 'fkComment' => $field->getComment() ? '(' . $field->getComment() . ')' : '', 
