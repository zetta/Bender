...
class Criteria
{
     ...
     * @param string [optional] $mutatorColumn
     * @return Criteria $this
     */
    public function add($column, $value, $comparison = '=', $mutator = '', $mutatorColumn = false)
    {
        if(!$comparison)
            throw new Exception('Se necesita especificar un signo de comparación para el criterio');
        
        if($comparison == self::IN && empty($value))
            $value = array(-1);
            ...
            foreach($this->keys() as $key)
            {
                ...
                if(isset($criterion['firstCriteria']) && isset($criterion['secondCriteria']))
                {
                    ...
                    else
                    {
                      ...
                      {
                           if($criterion['mutatorColumn'])
                               $criterion['column'] = sprintf($criterion['mutator'], $criterion['column']);
                           else 
